...
//  _titleText ctrlSetText format["%3 (%1%2)...",round(_cP * 100),"%",_title];
    ...
    if(speed player > 0) exitWith     {_exit = true};
    if(speed player < 0) exitWith     {_exit = true};
...
player playActionNow "stop";
    if(_exit) exitWith {[1,"Tu ne dois pas bouger"]call DA3F_fnc_hint};
    if!(alive player) exitWith {[1,"Tu ne dois pas bouger"]call DA3F_fnc_hint};
