...
_warcash= _this select 5;
_query = format ["SELECT pid, name, cash, bankacc, save_pos, spec, inv_virt, inv_unit, stats, adminlevel, hierarchie, cashwar,mdp_coms, mymsg FROM players WHERE pid='%1'",_uid];
...
if (isNil "_queryResult" || str _queryResult isEqualTo "[]") then
	...
	_bank 	= [_bank] call DB_fnc_numberSafe;
		_query = format ["INSERT INTO players (pid, name, cash, bankacc, aliases, spec, inv_virt, inv_unit, stats, cashwar, mymsg) VALUES('%1', '%2', '%3', '%4', '%5','""[]""','""[]""','""[]""','""[100,100,0,0]""','%6','""[]""')",
		    _uid,
		    _name,
		    _money,
		    _bank,
	...
	_queryResult remoteExecCall ["DA3F_fnc_result",_unit];
	_queryResult = [_query,1] call DB_fnc_asyncCall;
		diag_log format [" 2/3 retour CpRp :( query : %1 ) [uid : %2]", _queryResult,_uid];

	}];

		diag_log format [" 3/3 retour CpRp :( query : %1 ) [uid : %2]", _queryResult,_uid];