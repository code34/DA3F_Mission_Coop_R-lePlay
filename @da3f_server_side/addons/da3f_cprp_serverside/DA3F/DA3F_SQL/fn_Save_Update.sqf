...
_Wcash 		= _this select 7;
_mymsg 		= _this select 8;
_getpos		= _this select 9;
...
*/
	...
	_mymsg 	= [_mymsg] call DB_fnc_mresArray;
	_getpos = [_getpos] call DB_fnc_mresArray;

	_money 	= [_money] call DB_fnc_numberSafe;
	_bank 	= [_bank] call DB_fnc_numberSafe;
	_Wcash 	= [_Wcash] call DB_fnc_numberSafe;

	_query = format["UPDATE players SET cash='%2', bankacc='%3', spec='%4', inv_virt='%5', inv_unit='%6', stats='%7', cashwar='%8', mymsg='%9', save_pos='%10' WHERE pid='%1'",_uid,_cash,_bank,_spe_L,_invVirt,_invUnit,_stats,_Wcash,_mymsg,_getpos];
