...
_position = if (_side isEqualTo independent) then {[_position] call DB_fnc_mresArray} else {[]};
...
switch (_side) do {
    ...
    case independent: {_query = format ["UPDATE players SET name='%1', cash='%2', bankacc='%3', med_licenses='%4', med_gear='%5', med_stats='%6', playtime='%7',civ_position='%9' WHERE pid='%8'",_name,_cash,_bank,_licenses,_gear,_stats,_playtime_update,_uid,_position];};
};
