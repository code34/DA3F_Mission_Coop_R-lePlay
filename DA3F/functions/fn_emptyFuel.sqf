...
while{true} do
{
    ...
    if(isEngineOn _veh_Units && ((driver _veh_Units) == player) && (_veh_Units != player) && ((speed _veh_Units)> 0.2) ) then
    {
        ...
        {
            hint "Véhicule en réserve ! Allez faire le plein dans une base !";
        }
        else
        {
            if(fuel _veh_Units < 0.03) then
            {
                hint "Tu peux toujours essayer de pisser dans le réservoir, mais je te conseil d'aller dans une base au plus vite !";
            };
        };
